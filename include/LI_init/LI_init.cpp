...
LI_Init::LI_Init() : time_delay_IMU_wtr_Lidar(0.0), time_lag_1(0.0), time_lag_2(0.0), lag_IMU_wtr_Lidar(0) {
...
void LI_Init::fout_before_filter() {
  for (auto it_IMU = IMU_state_group.begin(); it_IMU != IMU_state_group.end() - 1; it_IMU++) {
    fout_before_filt_IMU << setprecision(15) << it_IMU->ang_vel.transpose() << " " << it_IMU->ang_vel.norm() << " " << it_IMU->linear_acc.transpose() << " "
                         << it_IMU->timeStamp << endl;
  }
  for (auto it = Lidar_state_group.begin(); it != Lidar_state_group.end() - 1; it++) {
    fout_before_filt_Lidar << setprecision(15) << it->ang_vel.transpose() << " " << it->ang_vel.norm() << " " << it->timeStamp << endl;
  }
}
...
void LI_Init::downsample_interpolate_IMU(const double &move_start_time) {
  ...
  for (int i = 0; i < Lidar_state_group.size(); i++) {
    for (int j = 1; j < IMU_state_group_ALL.size(); j++) {
      if (IMU_state_group_ALL[j - 1].timeStamp <= Lidar_state_group[i].timeStamp && IMU_state_group_ALL[j].timeStamp > Lidar_state_group[i].timeStamp) {
        ...
        IMU_state_interpolation.ang_vel    = s * IMU_state_group_ALL[j - 1].ang_vel + (1 - s) * IMU_state_group_ALL[j].ang_vel;
        IMU_state_interpolation.linear_acc = s * IMU_state_group_ALL[j - 1].linear_acc + (1 - s) * IMU_state_group_ALL[j].linear_acc;
        push_IMU_CalibState(IMU_state_interpolation.ang_vel, IMU_state_interpolation.linear_acc, Lidar_state_group[i].timeStamp);
        break;
      }
    }
  }
...
void LI_Init::central_diff() {
  ...
  for (; it_IMU_state != IMU_state_group.end() - 2; it_IMU_state++) {
    ...
    fout_IMU_meas << setprecision(12) << it_IMU_state->ang_vel.transpose() << " " << it_IMU_state->ang_vel.norm() << " " << it_IMU_state->linear_acc.transpose()
                  << " " << it_IMU_state->ang_acc.transpose() << " " << it_IMU_state->timeStamp << endl;
  }
  ...
  for (; it_Lidar_state != Lidar_state_group.end() - 2; it_Lidar_state++) {
    ...
    fout_LiDAR_meas << setprecision(12) << it_Lidar_state->ang_vel.transpose() << " " << it_Lidar_state->ang_vel.norm() << " "
                    << (it_Lidar_state->linear_acc - STD_GRAV).transpose() << " " << it_Lidar_state->ang_acc.transpose() << " " << it_Lidar_state->timeStamp
                    << endl;
  }
}
...
void LI_Init::solve_Rotation_only() {
  ...
  for (int i = 0; i < IMU_state_group.size(); i++) {
    ...
    problem_rot.AddResidualBlock(Angular_Vel_Cost_only_Rot::Create(IMU_state_group[i].ang_vel, Lidar_state_group[i].ang_vel), nullptr, R_LI_quat);
  }
...
void LI_Init::solve_Rot_bias_gyro(double &timediff_imu_wrt_lidar) {
  ...
  for (int i = 0; i < IMU_state_group.size(); i++) {
    double deltaT = Lidar_state_group[i].timeStamp - IMU_state_group[i].timeStamp;
    problem_ang_vel.AddResidualBlock(Angular_Vel_Cost::Create(IMU_state_group[i].ang_vel, IMU_state_group[i].ang_acc, Lidar_state_group[i].ang_vel, deltaT),
                                     nullptr, R_LI_quat, bias_g, &time_lag2);
  }
  ...
  cout << "Using LIO: SUBTRACT this value from IMU timestamp" << endl << "           or ADD this value to LiDAR timestamp." << endl << endl;

  // The second temporal compensation
  IMU_time_compensate(get_lag_time_2(), false);

  for (int i = 0; i < Lidar_state_group.size(); i++) {
    fout_after_rot << setprecision(12) << (Rot_Lidar_wrt_IMU * Lidar_state_group[i].ang_vel + gyro_bias).transpose() << " " << Lidar_state_group[i].timeStamp
                   << endl;
  }
}
...
void LI_Init::solve_trans_biasacc_grav() {
  ...
  for (int i = 0; i < IMU_state_group.size(); i++) {
    problem_acc.AddResidualBlock(Linear_acc_Cost::Create(Lidar_state_group[i], Rot_Lidar_wrt_IMU, IMU_state_group[i].linear_acc), nullptr, R_GL0_quat, bias_aL,
                                 Trans_IL);
  ...
  options_acc.linear_solver_type = ceres::DENSE_NORMAL_CHOLESKY;
  options_acc.max_num_iterations = 100;
  ceres::Solver::Summary summary_acc;

  std::cout << "[LI_Init] solve_trans_biasacc_grav: IMU_state_group size: " << IMU_state_group.size()
            << ", Lidar_state_group size: " << Lidar_state_group.size() << std::endl;

  ceres::Solve(options_acc, &problem_acc, &summary_acc);

  std::cout << "[LI_Init] Ceres solve summary: " << summary_acc.message << std::endl;
  std::cout << "[LI_Init] Initial bias_aL: [0, 0, 0], Final bias_aL: [" << bias_aL[0] << ", " << bias_aL[1] << ", " << bias_aL[2] << "]" << std::endl;
  ...
  for (int i = 0; i < IMU_state_group.size(); i++) {
    ...
    fout_acc_cost << setprecision(10) << acc_I.transpose() << " " << acc_L.transpose() << " " << IMU_state_group[i].timeStamp << " "
...
bool LI_Init::data_sufficiency_assess(MatrixXd &Jacobian_rot, int &frame_num, V3D &lidar_omg, int &orig_odom_freq, int &cut_frame_num) {
  ...
  if (frame_num % orig_odom_freq * cut_frame_num == 0) {
    ...
    V3D Rot_percent(Scaled_Eigen[1] * Scaled_Eigen[2], Scaled_Eigen[0] * Scaled_Eigen[2], Scaled_Eigen[0] * Scaled_Eigen[1]);

    V3D Rot_percent_scaled(Rot_percent[0] < 0.99 ? Rot_percent[0] : 1, Rot_percent[1] < 0.99 ? Rot_percent[1] : 1, Rot_percent[2] < 0.99 ? Rot_percent[2] : 1);
...
void LI_Init::LI_Initialization(int &orig_odom_freq, int &cut_frame_num, double &timediff_imu_wrt_lidar, const double &move_start_time) {
...
void LI_Init::print_initialization_result(double &time_L_I, M3D &R_L_I, V3D &p_L_I, V3D &bias_g, V3D &bias_a, V3D gravity) {
  ...
  cout << setprecision(6) << "Rotation LiDAR to IMU    = " << RotMtoEuler(R_L_I).transpose() * 57.3 << " deg" << endl;
  printf(BOLDCYAN "[Init Result] " RESET);
...
void LI_Init::plot_result() {
  ...
  for (auto it_Lidar_state = Lidar_state_group.begin(); it_Lidar_state != Lidar_state_group.end() - 1; it_Lidar_state++) {
    for (int i = 0; i < 3; i++) {
